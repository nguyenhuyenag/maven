...
for /f "usebackq tokens=1-4 delims=," %%a in ("%file%") do (
	...
	echo.
	call mvn install:install-file -Dfile=%LIB_DIR%\%%a -DgroupId=%%b -DartifactId=%%c -Dversion=%%d -Dpackaging=jar -Durl=file:./lib/ -DrepositoryId=lib -DupdateReleaseInfo=true
	echo.
)
